...
async function summarizeText(text, maxLength) {
    isItWorthTokenizing(cleanedText);
...
async function processTokenGroups(tokenGroups) {
    for (let group of tokenGroups) {
        ...
        // Append the summary as a user message and communicate with the OpenAI API
        // ...
    }
}
...
if (tokenCount > 4096) {
    ...
    await processTokenGroups(tokenGroups);
}
...
async function extractAutoTags($) {
