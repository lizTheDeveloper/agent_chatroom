...
const { processLinkDataData } = require('./linkDragonScribe.js');

// const OPENAI_API_KEY = os.getenv("OPENAI_API_KEY")
// const client = OpenAI(api_key=OPENAI_API_KEY)
const client = new OpenAI(base_url="http://192.168.1.178:1234/v1", model = "NousResearch/Hermes-2-Pro-Mistral-7B-GGUF/Hermes-2-Pro-Mistral-7B.Q4_0.gguf",api_key="lm-studio")
...
class VectorInsertion {
    ...
    async generateEmbeddingWithAI(embeddingVector) {
        try {
            const { content_summary } = embeddingVector;
