...
class retrieveLinkFromDatabase {
    ...
    async generateValuesWithAI(data) {
        const systemPrompt = `A user has made a request: "${userQuery}". First, determine whether this is a request to query the SQL database. Generate a SQL query to search for the following following: title, content summary, link name, uploaded by, upload date, tags.`;
