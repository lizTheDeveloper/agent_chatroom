...
class LinkRetrieval {
    ...
    async generateValuesWithAI(data) {
        const systemPrompt = `A user has made a request to retrieve data related to the following criteria: "${JSON.stringify(data)}". Please generate provide the values for each property using the following format "SELECT * FROM links WHERE title = $1 AND content_summary = $2 AND link_name = $3 AND uploaded_by = $4 AND upload_date = $5 AND tags = $6" and to fit an SQL query based on the user request.`;
        const chatCompletion = await OpenAI.createCompletion({
            model: "text-davinci-003",
            prompt: systemPrompt,
            temperature: 0.5,
