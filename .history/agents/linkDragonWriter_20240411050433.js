...
async function linkDragonMentioned(message) {
    ...
    if (fullResponse.includes("1)")) {
        ...
        const userDBQuery = fullResponse;
        return userDBQuery;
        await LinkDragonKeeper.retrieveLinkFromDatabase(userDBQuery);
    } else if (fullResponse.includes("2)")) {
        // Logic for querying the vector database
        const userVectorQuery = fullResponse;
        return userVectorQuery;
        await queryVectorDatabase(userVectorQuery);
...
async function handleLinkMessage(link, username) {
    try {
        const linkData = {
            link,
            username,
            timetimestamp: new Date().toISOString(),
        };
