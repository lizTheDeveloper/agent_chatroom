...
agent.registerMessageHandler("general", async function (msg) {
    ...
    if (msg.includes("@LinkDragon".toLowerCase)) {
        ...
        try {
            ...
            await processReponse(msg);
            //depending on the message, trigger one of three action flows- retrieve, save, or help
            sendMessageToChat()
        }
    }
});
...
function isRetrievalMessage(message) {
    const prompt = `Translate this user request into a GitHub CLI command: "${msg.replace("@LinkDragon", "").trim()}"`;
