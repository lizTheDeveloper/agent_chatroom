...
const { LinkInsertion } = require('./linkDragonKeeper.js');
// const tiktoken = require('tiktoken');
// let OPENAI_API_KEY = os.getenv("OPENAI_API_KEY")
const client = new OpenAI(base_url="http://192.168.1.178:1234/v1", api_key="lm-studio")

async function handleReceivedLink(linkData) {
    try {
        const processedData = await processLinkData(linkData);
        const linkInserter = new LinkInsertion();
        await linkInserter.insertLinkData(processedData);
        console.log("Link data processed and inserted successfully!");
