...
class VectorRetrieval {
    ...
    async generateQueryWithAI(userQuery) {
        const systemPrompt = `A user has made a request to retrieve links related to the following query: "${userQuery}". Please provide an SQL query that retrieves the relevant links from the vector database, ordered by their similarity to the query.`;
        const chatCompletion = await OpenAI.createCompletion({
            model: "text-davinci-003",
            prompt: systemPrompt,
            temperature: 0.5,
