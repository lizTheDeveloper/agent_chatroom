...
class retrieveLinkFromDatabase {
    ...
    async generateValuesWithAI(data) {
        const systemPrompt = `A user has made a request: "${userQuery}". First, determine whether this is a request to query the SQL database, or a request to query to the vector database.  Generate a SQL query  following: title, content summary, link name, uploaded by, upload date, tags.`;
        ...
        const query = chatCompletion.choices[0].text.trim();
