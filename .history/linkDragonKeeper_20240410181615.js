...
agent.connect();
agent.login();
class LinkDragonKeeper extends LinkDragonScribe {
    constructor(agentName, password, chatroom, socketUrl) {
        super(agentName, password, chatroom, socketUrl);
        // Register the message handler
        this.registerMessageHandler('linkdragonkeeper', this.handleReceivedLink.bind(this));
    }
}

const linkDragonKeeper = new LinkDragonKeeper(agentName, password, chatroom, socketUrl);
linkDragonKeeper.connect();
linkDragonKeeper.login();
