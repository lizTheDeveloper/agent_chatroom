...
const agentName = 'LinkDragonWriter';
const password = 'your_password'; //amend to password from users.json
...
class LinkDragonWriter extends Agent {
    ...
    async handleMessage(msg) {
        ...
        if (links) {
            for (const link of links) {
                ...
                if (isEmbeddable) {
                    // Extract user-defined tags from the message
                    const tags = this.extractTags(restOfMessage);
    
                    // Send the link and tags to LinkDragonScribe for scraping, parsing, and storing
                    this.sendLinkToScribe(link, username, tags);
                }
            }
        }
    }
    ...
    sendLinkToScribe(link, username) {
